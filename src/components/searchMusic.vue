...
const searchNameOrId = _.debounce(function (isDownload: boolean = false) {
  var input: string = yourInput.value.trim().slice(0, 20);
  if (input) {
      ...
      api.apiMusicList.searchMusicList(input).then(
        res => {
          if (res.data.code == 200)
            download.value = res.data.data[0].url
        },
        err => {
          console.log(err.message)
        }
      )
      api.apiMusicList.searchMusicListNew(input).then(res => {
        console.log(res.data.code, typeof res.data.code)
        if (res.data.code == 200)
          download1.value = res.data.data[0].url
      },
        err => {
          console.log(err.message)
        }
...
<template>
  ...
  <div class="search">
    <el-autocomplete style="width: 520px;" v-model="yourInput" :fetch-suggestions="querySearchAsync"
      fit-input-width:true placeholder="Please input what you want" :clearable="true" @keyup.enter="SearchMusic"
      @select="handleSelect" />
  </div>
  <div class="card">
    <button type="button" @click="count++">count is {{ countPlus }}</button>
  </div>
  <p v-if="download">
    Check out
